...
router.get('/adduser', (req, res) => {
    res.render('addUser', {title: "ALL ASSETS", msg:''})
});
...
router.post('/adduser',(req, res) =>{
    ...
    User.save().then(()=>{
        res.render('add', {title: "ALL ASSETS", msg:'Added Successfully'})
    }).catch((err)=>{
        console.log(err)
    })
})
...
router.get('/data',(req,res) => {
    ...
    allData.exec((err, data) => {
        if(err){
            ...
            res.render('viewUserAssets',{title: "View Users Assets", data:data});
        }
    })
